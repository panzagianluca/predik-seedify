...
contract Outcome1155 is ERC1155, AccessControl {
    ...
    /// @notice HIGH-3 FIX: Pending router upgrade with timelock
    ...
    /// @notice HIGH-3 FIX: Timelock duration for router upgrades (48 hours)
    ...
    event RouterUpgradeProposed(address indexed newRouter, uint64 effectiveAt);
    event RouterUpgradeCancelled(address indexed cancelledRouter);
    ...
    /// @notice HIGH-3 FIX: Propose router upgrade with 48-hour timelock
    /// @param newRouter Address of the new router
    function proposeRouterUpgrade(address newRouter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newRouter != address(0), "Outcome1155: router required");
        
        pendingRouterUpgrade = RouterUpgrade({
            newRouter: newRouter,
            effectiveAt: uint64(block.timestamp + ROUTER_UPGRADE_DELAY)
        });
        
        emit RouterUpgradeProposed(newRouter, pendingRouterUpgrade.effectiveAt);
    }

    /// @notice HIGH-3 FIX: Execute pending router upgrade after timelock expires
    function executeRouterUpgrade() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pendingRouterUpgrade.newRouter != address(0), "Outcome1155: no pending upgrade");
        require(
            block.timestamp >= pendingRouterUpgrade.effectiveAt,
            "Outcome1155: timelock active"
        );
    ...
    /// @notice HIGH-3 FIX: Cancel pending router upgrade
    function cancelRouterUpgrade() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pendingRouterUpgrade.newRouter != address(0), "Outcome1155: no pending upgrade");
        
        address cancelled = pendingRouterUpgrade.newRouter;
        delete pendingRouterUpgrade;
        
        emit RouterUpgradeCancelled(cancelled);
    }

    /// @notice LEGACY: Direct router update (DEPRECATED - use proposeRouterUpgrade instead)
    /// @dev Kept for initial setup only, should use timelock for production upgrades
