...
/**
 * @title MockUSDT (USDPredik)
 * @notice USDPredik - Platform currency for Predik prediction markets on BNB Mainnet
...
contract MockUSDT is ERC20, Ownable {
    ...
    /// @notice Amount minted per faucet call (10,000 USDPredik)
    uint256 public constant FAUCET_AMOUNT = 10_000 * 10 ** DECIMALS;

    /// @notice Initial mint amount for deployer (1,000,000 USDPredik)
     ...
     */
    constructor() ERC20("USDPredik", "USDp") Ownable(msg.sender) {
    ...
    /**
     * @notice Faucet function - Mints 10,000 USDPredik to caller
     * @dev Has 1-hour cooldown to prevent abuse
     */
    function faucet() external {
        uint256 lastClaim = lastFaucetClaim[msg.sender];
        if (lastClaim != 0) {
            require(block.timestamp >= lastClaim + FAUCET_COOLDOWN, "USDPredik: Faucet cooldown active");
        }
