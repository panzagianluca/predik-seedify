...
contract Oracle is AccessControl, ReentrancyGuard {
    ...
    /// @notice CRITICAL-4 FIX: Emergency resolution delay (48 hours after trading ends)
    uint64 public constant EMERGENCY_RESOLUTION_DELAY = 48 hours;

    /// @notice CRITICAL-4 FIX: Emergency withdrawal delay (7 days after trading ends)
    ...
    event EmergencyResolution(address indexed market, uint8 outcome, bool invalid, address indexed resolver);
    event EmergencyWithdrawal(address indexed market, address indexed user, uint256 amount);
    ...
    error Oracle_EmergencyDelayNotMet(uint64 currentTime, uint64 requiredTime);
    error Oracle_MarketNotStuck(address market);
    error Oracle_NoSharesToWithdraw(address market, address user);
    ...
    // -------------------------------------------------------------------------
    // CRITICAL-4 FIX: Emergency Resolution Mechanisms
    // -------------------------------------------------------------------------

    /// @notice Emergency manual resolution if DelphAI fails to resolve within 48 hours
    /// @dev Can only be called by admin if DelphAI hasn't resolved after EMERGENCY_RESOLUTION_DELAY
    /// @param market Address of the stuck market
    /// @param outcome Winning outcome index (or any outcome if marking invalid)
    /// @param invalid Whether to mark market as invalid (refunds all participants)
        ...
        nonReentrant
    {
        ...
        // Must be stuck in Pending state
        if (resolution.status != ResolutionStatus.Pending) {
            revert Oracle_MarketNotStuck(market);
        }
        
        // Must wait 48 hours after trading ends
        uint64 tradingEndsAt = ILMSRMarket(market).tradingEndsAt();
        uint64 emergencyTime = tradingEndsAt + EMERGENCY_RESOLUTION_DELAY;
        
        if (block.timestamp < emergencyTime) {
            revert Oracle_EmergencyDelayNotMet(uint64(block.timestamp), emergencyTime);
        }
        
        // Validate outcome
        uint8 outcomeCount = ILMSRMarket(market).outcomeCount();
        if (!invalid && outcome >= outcomeCount) {
            revert Oracle_InvalidOutcome(outcome);
        }
        
        // Set manual resolution with 100% confidence (admin override)
        resolution.proposedOutcome = outcome;
        resolution.invalid = invalid;
        resolution.confidence = 100; // Admin manual resolution
        resolution.proposedAt = uint64(block.timestamp);
        resolution.status = ResolutionStatus.Finalized; // Mark as finalized immediately (no dispute window)
        
        // Call the market's finalize function directly
    ...
    /// @notice Emergency withdrawal if market is stuck for 7+ days without resolution
    /// @dev Allows users to withdraw proportionally if DelphAI and admin both fail
    /// @param market Address of the stuck market
    /// @dev This marks the market as invalid and refunds users pro-rata
    function emergencyWithdraw(address market) external nonReentrant {
        Resolution storage resolution = resolutions[market];
        
        // Must still be in Pending state (not resolved, not disputed)
        if (resolution.status != ResolutionStatus.Pending) {
            revert Oracle_MarketNotStuck(market);
        }
        
        // Must wait 7 days after trading ends
        uint64 tradingEndsAt = ILMSRMarket(market).tradingEndsAt();
        uint64 emergencyTime = tradingEndsAt + EMERGENCY_WITHDRAWAL_DELAY;
        
        if (block.timestamp < emergencyTime) {
            revert Oracle_EmergencyDelayNotMet(uint64(block.timestamp), emergencyTime);
        }
        
        // Mark market as invalid (first withdrawal triggers this)
        if (resolution.proposedOutcome == 0 && resolution.proposedAt == 0) {
            resolution.proposedOutcome = 0; // Doesn't matter for invalid markets
            resolution.invalid = true;
            resolution.confidence = 0;
            resolution.proposedAt = uint64(block.timestamp);
            resolution.status = ResolutionStatus.Finalized;
            
            // Finalize market as invalid
            ILMSRMarket(market).finalize(0, true);
            
            emit EmergencyWithdrawal(market, msg.sender, 0);
            emit ResolutionFinalized(market, 0, true);
        }
        
        // Note: Actual withdrawal happens in LMSRMarket.redeem()
        // Users call redeem() after market is finalized as invalid
    }
...
interface ILMSRMarket {
    ...
    function tradingEndsAt() external view returns (uint64); // CRITICAL-4 FIX
}
