...
contract Treasury is AccessControl, ReentrancyGuard {
    ...
    /// @notice HIGH-4 FIX: Role for reporting fees (MarketFactory has this)
    bytes32 public constant FEE_REPORTER_ROLE = keccak256("FEE_REPORTER_ROLE");

    /// @notice HIGH-4 FIX: Role for withdrawing fees (Only treasury admin has this)
    ...
    constructor(uint256 protocolBps_, uint256 creatorBps_, uint256 oracleBps_, address oracle_) {
        ...
        _grantRole(FEE_WITHDRAWER_ROLE, msg.sender); // HIGH-4 FIX: Admin can withdraw
        // NOTE: FEE_REPORTER_ROLE is granted to MarketFactory during deployment
    }
    ...
    /// @dev HIGH-4 FIX: Now requires FEE_REPORTER_ROLE instead of checking msg.sender == market
    function collect(uint256 marketId, address token, uint256 amount) 
        external 
        nonReentrant
        onlyRole(FEE_REPORTER_ROLE)
    {
        if (!markets[marketId].registered) {
        ...
        address marketAddress = markets[marketId].marketAddress;
        ...
        IERC20(token).safeTransferFrom(marketAddress, address(this), amount);
        ...
        emit FeesCollected(marketId, marketAddress, token, protocolAmount, creatorAmount, oracleAmount, amount);
    }
    ...
    /// @dev HIGH-4 FIX: Now requires FEE_WITHDRAWER_ROLE instead of PROTOCOL_ROLE
