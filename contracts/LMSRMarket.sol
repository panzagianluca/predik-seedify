...
contract LMSRMarket is AccessControl, ReentrancyGuard {
    ...
    /// @notice Emitted when approved router is updated.
    event ApprovedRouterUpdated(address indexed router, bool approved);
    ...
    error LMSR_OnlyRouterAllowed(address caller);
    error LMSR_SlippageExceeded(uint256 actual, uint256 limit);
    error LMSR_MaxCostExceeded(uint256 actualCost, uint256 maxCost);
    error LMSR_MinPayoutNotMet(uint256 actualPayout, uint256 minPayout);
    ...
    /// @notice HIGH-1 FIX: Approved routers for slippage protection
    ...
    /// @notice HIGH-1 FIX: Only approved routers can call buy/sell (slippage protection)
    modifier onlyRouter() {
        if (!approvedRouters[msg.sender]) {
            revert LMSR_OnlyRouterAllowed(msg.sender);
        }
        _;
    }
    ...
    /// @notice HIGH-1 FIX: Set approved router for slippage-protected trading
    /// @param router Address of the router contract
    /// @param approved Whether this router is approved
        ...
        returns (uint256 totalPaid)
    {
        ...
        // AUDIT FIX: Slippage protection - revert if actual cost exceeds max
        ...
        returns (uint256 netPayout)
    {
        ...
        // AUDIT FIX: Slippage protection - revert if actual payout is below minimum
