...
contract VerifyDeployment is Script {
    ...
    function verifyMockUSDT(MockUSDT _mockUSDT) internal view {
        ...
        // Test 1: Check name
        require(keccak256(bytes(_mockUSDT.name())) == keccak256(bytes("Mock USDT")), "MockUSDT: Invalid name");
        console.log("  [PASS] Name: Mock USDT");

        // Test 2: Check symbol
        require(keccak256(bytes(_mockUSDT.symbol())) == keccak256(bytes("USDT")), "MockUSDT: Invalid symbol");
    ...
    function verifyIntegration(
        ...
        require(_oracle.hasRole(adminRole, MARKET_FACTORY), "Integration: Factory doesn't have admin role in Oracle");
        console.log("  [PASS] Factory has admin role in Oracle");

        // Test 2: Factory has admin role in Treasury
        require(
            _treasury.hasRole(adminRole, MARKET_FACTORY), "Integration: Factory doesn't have admin role in Treasury"
        );
        console.log("  [PASS] Factory has admin role in Treasury");

        // Test 3: Router is set in Outcome1155
        require(_outcome1155.router() == ROUTER, "Integration: Router not set in Outcome1155");
        console.log("  [PASS] Router properly set in Outcome1155");

        // Test 4: All contracts reference correct collateral
        require(
            address(_factory.collateral()) == MOCK_USDT && address(_oracle.collateral()) == MOCK_USDT,
        ...
        require(address(_factory.oracle()) == ORACLE, "Integration: Oracle reference mismatch");
        console.log("  [PASS] Factory references correct Oracle");

        // Test 6: All contracts reference correct Treasury
        require(
            address(_factory.treasury()) == TREASURY && _oracle.treasury() == TREASURY,
