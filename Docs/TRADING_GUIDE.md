...
**Status:** ✅ Tested and Working (with Slippage Protection)

---

## 📋 Quick Reference

### Contract Addresses (October 29, 2025 Deployment)

```bash
# Core Contracts
MARKET_FACTORY=0x9Be256DDB94Cd1563738c9c77Fca3c62e7321A5E
USDT=0x065b8ADad86048edC320277e92C58a4EEB3Bf902
OUTCOME_1155=0xB5c0e214F8D3f3f1DA4897418AC4C4458ee420c0
ROUTER=0x321F34Ac6404BE162bD79a8A1B4abDBbFC92e84a

# Example Market (Market ID 0)
MARKET_0=0xD4646283c2e5A686AdD3E74E920F3dfB906116E0
```
...
### `buy` - Buy Shares (with Slippage Protection)

Purchase shares for a specific outcome with optional slippage protection.

**⚠️ IMPORTANT:** As of October 29, 2025, direct market calls are BLOCKED. You MUST use the Router for all trades.

```solidity
function buy(uint8 outcomeId, uint256 deltaShares, uint256 maxCost) 
    external 
    onlyRouter  // ← Direct calls will revert!
    returns (uint256 totalPaid); // Amount of USDT spent (6 decimals)
```

**Parameters:**
- `outcomeId`: Which outcome to buy (0, 1, 2, etc.)
- `deltaShares`: How many shares to buy (18 decimals)
- `maxCost`: Maximum USDT willing to spend (6 decimals). Pass `0` for no limit.

**Slippage Protection:**
If `maxCost > 0` and actual cost exceeds it, transaction reverts with `LMSR_MaxCostExceeded`.
...
uint256 maxCost = (totalCost * 101) / 100; // Allow up to 1% more

// Approve Router (not market!)
usdt.approve(routerAddress, maxCost);

// Buy via Router (Router forwards to market with slippage check)
router.buyWithPermit(..., maxCost, ...);
```
```
