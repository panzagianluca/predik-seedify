...
contract MarketFactoryTest is Test {
    ...
    function test_CreateMarket_WithCustomParameters() public {
        ...
        MarketFactory.CreateMarketParams memory params = MarketFactory.CreateMarketParams({
            title: "Custom Market",
            description: "Test market description",
            category: "Test",
            imageUrl: "",
            outcomes: outcomes,
            tradingEndsAt: tradingEndsAt,
            liquidityParameter: customB,
            protocolFeeBps: customProtocolFee,
            creatorFeeBps: customCreatorFee,
            oracleFeeBps: 0,
            initialLiquidity: initialLiquidity,
            delphAIMarketId: 2
        });
    ...
    function test_CreateMarket_MultipleMarkets() public {
        ...
        (uint256 id1, address addr1) =
            factory.createMarket(_defaultParams("Market 1", tradingEndsAt, initialLiquidity, 10));
        (uint256 id2, address addr2) =
            factory.createMarket(_defaultParams("Market 2", tradingEndsAt, initialLiquidity, 11));
        (uint256 id3, address addr3) =
            factory.createMarket(_defaultParams("Market 3", tradingEndsAt, initialLiquidity, 12));
        vm.stopPrank();
    ...
    function test_CreateMarket_RevertIf_Unauthorized() public {
        vm.prank(user);
        vm.expectRevert();
        factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 1000e6, 20));
    }
    ...
    function test_CreateMarket_RevertIf_TradingEndsInPast() public {
        ...
        factory.createMarket(_defaultParams("Test", uint64(block.timestamp - 1), 1000e6, 22));
    }

    function test_CreateMarket_RevertIf_ZeroLiquidity() public {
        vm.prank(admin);
        vm.expectRevert();
        factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 0, 23));
    }
    ...
    function test_CreateMarket_RevertIf_InvalidCustomFees() public {
        ...
        MarketFactory.CreateMarketParams memory params = MarketFactory.CreateMarketParams({
            ...
            imageUrl: "",
            outcomes: outcomes,
            tradingEndsAt: uint64(block.timestamp + 1 days),
            liquidityParameter: 0,
            protocolFeeBps: 6000,
            creatorFeeBps: 4000,
            oracleFeeBps: 1000,
    ...
    function test_GetMarket_Success() public {
        ...
        (uint256 marketId, address marketAddress) =
            factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 10000e6, 30));
        vm.stopPrank();
    ...
    function test_GetMarketId_Success() public {
        ...
        usdt.approve(address(factory), 10000e6);
        (uint256 marketId, address marketAddress) =
            factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 10000e6, 31));
        vm.stopPrank();
    ...
    function test_GetAllMarkets() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            (, address addr) =
                factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 10000e6, 40 + i));
            created[i] = addr;
        }
        vm.stopPrank();
    ...
    function test_GetMarkets_Pagination() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            factory.createMarket(_defaultParams("Test", uint64(block.timestamp + 1 days), 10000e6, 50 + i));
        }
        vm.stopPrank();
    ...
    function test_Integration_CreateAndTradeOnMarket() public {
        ...
        (, address marketAddress) =
            factory.createMarket(_defaultParams("Test Market", tradingEndsAt, initialLiquidity, 100));
        vm.stopPrank();
        ...
        market.buy(0, 100e6, 0); // Buy 100 shares of outcome 0
        vm.stopPrank();
